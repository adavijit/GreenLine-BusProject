 ...
 $(document).ready(function(){
  ...
  $(document).on('blur', '.update', function(){
   ...
   html += '<td contenteditable id="data2"></td>';
	 html += '<td contenteditable id="data3">'+generatePassword()+'</td>';
  ...
  $(document).on('click', '#insert', function(){
   var ad_name = $('#data1').text();
   var ad_user_name = $('#data2').text();
	 var ad_password = $('#data3').text();

   if(ad_name != '' && ad_user_name != '' && ad_password!= '')
   {
    $.ajax({
     url:"insertAdmin.php",
     method:"POST",
     data:{ad_name:ad_name, ad_user_name:ad_user_name,ad_password:ad_password},
     success:function(data)
     {
      $('#alert_message').html('<div class="alert alert-success">'+data+'</div>');
      $('#user_data').DataTable().destroy();
